...
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.Separator;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.append;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.charset;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.charsetName;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.formatClassName;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.formatPropertyName;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.generateFlyweightPropertyJavadoc;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.generateGroupEncodePropertyJavadoc;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.generateLiteral;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.generateOptionDecodeJavadoc;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.generateOptionEncodeJavadoc;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.generateTypeJavadoc;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.isAsciiEncoding;
import static uk.co.real_logic.sbe.generation.ts.TypeScriptUtil.typeScriptTypeName;
...
public class TypeScriptGenerator implements CodeGenerator
{
    ...
    {
        try (Writer out = outputManager.createOutput("codecs"))
        {
            final Set<String> types = new HashSet<>();
            final HashMap<String, Set<String>> typeGroups = new HashMap<>();
            ...
            for (final List<Token> tokens : ir.types())
            {
                ...
                types.add(type.applicableTypeName());
            }

            for (final List<Token> tokens : ir.messages())
            {
                final Token msgToken = tokens.get(0);
                types.add(msgToken.applicableTypeName());
                final List<Token> messageBody = getMessageBody(tokens);
                int i = 0;
                ...
                if (shouldImportsGroupsInIndexFile)
                {
                    final List<Token> groups = new ArrayList<>();
                    i = collectGroups(messageBody, i, groups);
                    for (final Token group : groups)
                    {
                        if (group.signal() == Signal.BEGIN_GROUP)
                        {
                            if (!typeGroups.containsKey(msgToken.applicableTypeName()))
                            {
                                final String groupName = group.applicableTypeName();
                                typeGroups.put(msgToken.applicableTypeName(), new HashSet<>()
                                {
                                    {
                                        add(groupName);
                                    }
                            ...
                            {
                                typeGroups.get(msgToken.applicableTypeName()).add(group.applicableTypeName());
                            }
                        }
                    }
                }
            ...
            {
                out.append("import { ")
                    .append(formatClassName(name)).append(", get").append(formatClassName(name)).append(", get")
                    .append(formatClassName(name)).append("Value").append(", get")
                    .append(formatClassName(name)).append("String")
                    .append(" } from './")
                    .append(formatClassName(name))
                    .append(".g';\n");
            ...
            for (final String type : types)
            {
                ...
                if (shouldImportsGroupsInIndexFile && typeGroups.containsKey(type))
                {
                    for (final String groupName : typeGroups.get(type))
                    {
                        out.append(",\n ").append(decoderName(groupName))
                            .append(" as ")
                            .append(decoderName(type)).append("_").append(decoderName(groupName));
                ...
                if (shouldImportsGroupsInIndexFile && typeGroups.containsKey(type))
                {
                    for (final String groupName : typeGroups.get(type))
                    {
                        out.append(",\n ").append(encoderName(groupName))
                            .append(" as ")
                            .append(encoderName(type)).append("_").append(encoderName(groupName));
        ...
        final boolean isSubGroup) throws IOException
    {
        for (int i = 0, size = tokens.size(); i < size; i++)
        {
            ...
            appendGroupInstanceDecoderDisplay(sb, fields, groups, varData, indent + INDENT, groupName);
        ...
        final String fqBuffer)
    {
        ...
        return "/* Generated SBE (Simple Binary Encoding) message codec. */\n" +
            ...
            "import * as codecs from \"./codecs.g\";\n" +
            interfaceImportLine() +
            importStatements;
    }
    ...
    private CharSequence generateWrapFieldNotPresentCondition(final int sinceVersion, final String indent)
    {
        ...
        return
            ...
            indent + "        {\n" +
            indent + "            wrapBuffer.wrap(this.buffer(), this._offset, 0);\n" +
            indent + "            return;\n" +
            indent + "        }\n\n";
    }
        ...
        final String indent)
    {
        ...
        else if (encoding.primitiveType() == PrimitiveType.UINT8)
        {
            ...
            new Formatter(sb).format("\n" +
                indent + "    public get%sFromBuffer(dst: %s, dstOffset: number, length: number): number\n" +
                indent + "    {\n" +
                "%s" +
                indent + "        const bytesCopied: number = Math.min(length, %d);\n" +
                indent + "        this.buffer().getBytes(this._offset + %d, dst, dstOffset, bytesCopied);\n\n" +
        ...
        final StringBuilder sb)
    {
        ...
        {
            new Formatter(sb).format("\n" +
                indent + "    public %1$sString(src: string): %2$s\n" +
                indent + "    {\n" +
        ...
        else
        {
            new Formatter(sb).format("\n" +
                ...
                indent + "    {\n" +
                indent + "        const length: number = %d;\n" +
                indent + "        const bytes: number[] = (!src || src === '') ?" +
                "  [] : Array.from(new TextEncoder().encode(src));\n" +
        ...
        final StringBuilder sb)
    {
        ...
        new Formatter(sb).format("\n" +
            indent + "    public put%s(src: %s, srcOffset: number, length: number): %s\n" +
            indent + "    {\n" +
            indent + "        if (length > %d)\n" +
            indent + "        {\n" +
            indent + "            throw new Error(" +
        ...
        final String indent)
    {
        ...
        else
        {
            ...
            new Formatter(sb).format(
                ...
                propertyName,
                CODEC_IMPORT_PATH + enumName,
                generatePropertyNotPresentCondition(inComposite, DECODER, fieldToken, CODEC_IMPORT_PATH + enumName,
                indent),
                rawGetStr,
                enumName);
        }
        ...
        final String bitSetName)
    {
        ...
        new Formatter(sb).format("\n" +
            ...
            indent + "    }\n",
            propertyName,
            CODEC_IMPORT_PATH + bitSetName + (notPresentCondition.isEmpty() ? "" : " | null"),
            notPresentCondition,
            propertyName,
            bitsetToken.offset(),
            propertyName);
        ...
        final String compositeName)
    {
        ...
        new Formatter(sb).format("\n" +
            ...
            indent + "    }\n",
            propertyName,
            CODEC_IMPORT_PATH + compositeName + (notPresentCondition.isEmpty() ? "" : " | null"),
            notPresentCondition,
            propertyName,
    ...
    private String generateChoiceIsEmptyInner(final PrimitiveType type)
    {
        switch (type)
        {
            ...
            case UINT64:
                return "0n == this.buffer().getLong(this._offset)";
    ...
    private String generateChoiceGet(final PrimitiveType type, final String bitIndex, final String byteOrder)
    {
        switch (type)
        {
            ...
            case UINT64:
                return "0n != (this.buffer().getLong(this._offset" + byteOrder + ") & (1n << " + bitIndex + "n))";
        ...
        switch (type)
        {
            ...
            case UINT64:
                return
                    ...
                    "        bits = value ? bits | (1n << " + bitIdx + "n) : bits & ~(1n << " + bitIdx + "n);\n" +
    ...
    private String generateStaticChoicePut(final PrimitiveType type, final String bitIdx)
    {
        switch (type)
        {
            ...
            case UINT64:
                return
                    "        return value ? bits | (1n << " + bitIdx + "n) : bits & ~(1n << " + bitIdx + "n);\n";
        ...
        switch (typeToken.signal())
        {
            ...
            case BEGIN_COMPOSITE:
            {
                ...
                append(sb, indent, "const " + fieldName + ": " +
                    CODEC_IMPORT_PATH + typeName + " | null = this." + fieldName + "();");
