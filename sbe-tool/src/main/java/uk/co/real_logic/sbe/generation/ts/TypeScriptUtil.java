...
/**
 * Utilities for mapping between {@link uk.co.real_logic.sbe.ir.Ir} and the TypeScript language.
 */
public class TypeScriptUtil
{
    ...
    /**
     * Map the name of a {@link PrimitiveType} to a JS primitive type name.
     *
    ...
    {
        final String charsetName = STD_CHARSETS.get(encoding);
        return Objects.requireNonNullElseGet(charsetName,
            () -> "\"" + (Charset.isSupported(encoding) ? Charset.forName(encoding).name() : encoding) + "\"");
    }
    ...
    public static String generateLiteral(final PrimitiveType type, final String value)
    {
        ...
        literal = switch (type)
        {
            case CHAR, UINT8, INT8, INT16 -> "(<" + castType + ">" + value + ")";
            case UINT16, INT32, UINT32 -> value;
            case FLOAT -> value; //value.endsWith("NaN") ? "Float.NaN" : value + "f";
            case INT64 -> value + "n";
            case UINT64 -> value + "n"; //0x" + Long.toHexString(Long.parseLong(value)) + "n";
            case DOUBLE -> value; //value.endsWith("NaN") ? "Double.NaN" : value + "d";
        };
